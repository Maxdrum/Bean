 ...
 */
"use strict";
angular.module('ZJSY_Admin').controller('ProfileController', function($scope,$http,$state,$stateParams,$rootScope){
    $scope.id ="";
    $scope.username = "";
    $scope.phone = "";
    $scope.email = "";
    ...
    $scope.changePwdwithOld = function(){
        ...
        $http.post(updatePwdApi,{
                ...
                if(data.code == 400) {
                    $rootScope.$broadcast('alerts',{type:'danger',message:"原密码错误，请重试！"});
                    //alert('原密码错误，请重试！')
                }
            })
